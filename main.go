...
var order_button_pressed_channel = make(chan config.OrderButon)
...
func main() {
	...
	err := network.Init(network.LocalListenPort, network.BroadcastListenPort, 1024,
...
func PollingHandler() {
	for {
		...
		case orderButton := <- order_button_pressed_channel:
			//Do...
			network_sending_channel <- network.Packet{Receiver_address: masterIP,
...
func NetworkHandler() {
	for {
		...
		fmt.Println("A button was pressed at "+Sender_address+"\n)
	}
} 
